...
const int MAX_RAM_PER_USER = 2048, MAX_STORAGE_PER_USER = 10240, DEFAULT_RAM = 100, DEFAULT_STORAGE = 1024, MIN_PASSWORD_LENGTH = 4;
const string AUTH_CSV_FILE = "auth.csv", WORK_DIR = "user_files";
void printHeader(const string& title) {
    cout << "\n╔════════════════════════════════════════════════════════╗\n";
    cout << "║  " << string((50 - title.length()) / 2, ' ') << title << string((51 - title.length()) / 2, ' ') << "║\n";
    cout << "╚════════════════════════════════════════════════════════╝\n";
}
void printSuccess(const string& message) { cout << "✓ " << message << "\n"; }
void printError(const string& message) { cout << "✗ " << message << "\n"; }
void printInfo(const string& message) { cout << "ℹ " << message << "\n"; }
string encodeBase64(const string& input) {
    string cmd = "echo -n \"" + input + "\" | base64";
    FILE* pipe = popen(cmd.c_str(), "r");
    if (!pipe) throw runtime_error("Failed to execute base64 encoding command");
    string result;
    char buffer[128];
    while (fgets(buffer, sizeof(buffer), pipe) != nullptr) result += buffer;
    if (pclose(pipe) != 0) throw runtime_error("Base64 encoding failed");
...
string decodeBase64(const string& input) {
    ...
    FILE* pipe = popen(cmd.c_str(), "r");
    if (!pipe) throw runtime_error("Failed to execute base64 decoding command");
    string result;
    char buffer[128];
    while (fgets(buffer, sizeof(buffer), pipe) != nullptr) result += buffer;
    if (pclose(pipe) != 0) throw runtime_error("Base64 decoding failed");
    return result;
}
bool isValidPassword(const string& password) {
    if (password.length() < MIN_PASSWORD_LENGTH) return false;
    for (char c : password) if (isdigit(c)) return true;
    return false;
}
...
public:
    ...
    User(const string& u, const string& p, const string& r) : username(u), password(p), role(r) {
        if (!isValidPassword(p)) throw invalid_argument("Password must be at least 4 characters with at least one number");
        if (r != "Admin" && r != "Employee") throw invalid_argument("Role must be 'Admin' or 'Employee'");
    }
    string getUsername() const { return username; }
    string getPassword() const { return password; }
    string getRole() const { return role; }
    void setPassword(const string& newPassword) {
        if (!isValidPassword(newPassword)) throw invalid_argument("Password must be at least 4 characters with at least one number");
...
private:
    unordered_map<string, int> ramAllocations, storageAllocations;
public:
    ...
    void allocateRam(const string& username, int amount) {
        if (amount < 0 || amount > MAX_RAM_PER_USER) throw out_of_range("RAM must be between 0 and " + to_string(MAX_RAM_PER_USER) + " MB");
        ramAllocations[username] = amount;
    }
    void allocateStorage(const string& username, int amount) {
        if (amount < 0 || amount > MAX_STORAGE_PER_USER) throw out_of_range("Storage must be between 0 and " + to_string(MAX_STORAGE_PER_USER) + " MB");
        storageAllocations[username] = amount;
    }
    int getRam(const string& username) const {
        auto it = ramAllocations.find(username);
        if (it != ramAllocations.end()) return it->second;
    ...
    int getStorage(const string& username) const {
        ...
        if (it != storageAllocations.end()) return it->second;
        throw runtime_error("User not found in resource manager");
    }
    ...
    unordered_map<string, int> getRamMap() const { return ramAllocations; }
    unordered_map<string, int> getStorageMap() const { return storageAllocations; }
};
...
public:
    ...
    void createFile(const string& filename) {
        if (!isPathSafe(filename)) throw runtime_error("Access denied: File path is outside allowed directory");
        fs::path filePath = workingDirectory / filename;
    ...
    void deleteFile(const string& filename) {
        if (!isPathSafe(filename)) throw runtime_error("Access denied: File path is outside allowed directory");
        fs::path filePath = workingDirectory / filename;
        if (!fs::exists(filePath)) throw runtime_error("File not found: " + filename);
        if (fs::remove(filePath) == 0) throw runtime_error("Failed to delete file: " + filename);
    }
    vector<string> listFiles() {
        vector<string> files;
        try {
            for (const auto& entry : fs::directory_iterator(workingDirectory))
                if (fs::is_regular_file(entry)) files.push_back(entry.path().filename().string());
    ...
    int getFileSize(const string& filename) {
        if (!isPathSafe(filename)) throw runtime_error("Access denied: Cannot access this file");
        fs::path filePath = workingDirectory / filename;
        if (!fs::exists(filePath)) throw runtime_error("File not found: " + filename);
...
public:
    ...
    void initializeCSV() {
        if (!fs::exists(csvFile)) {
            ...
            if (!file) throw runtime_error("Failed to create auth.csv");
            file.close();
    ...
    void loadUsers() {
        users.clear();
        ifstream file(csvFile);
        if (!file) throw runtime_error("Failed to open auth.csv");
        string line;
        while (getline(file, line)) {
            if (line.empty()) continue;
            stringstream ss(line);
            string username, encodedPassword, role, ramStr, storageStr;
            if (!getline(ss, username, ',') || !getline(ss, encodedPassword, ',') || !getline(ss, role, ',') || !getline(ss, ramStr, ',') || !getline(ss, storageStr, ',')) continue;
            try {
                string decodedPassword = decodeBase64(encodedPassword);
                int ram = stoi(ramStr), storage = stoi(storageStr);
    ...
    void saveUsers() {
        ofstream file(csvFile);
        if (!file) throw runtime_error("Failed to open auth.csv for writing");
    ...
    vector<User> getAllUsers() {
        ...
        for (const auto& pair : users) userList.push_back(pair.second);
        return userList;
    }
    ResourceManager& getResourceManager() { return resourceManager; }
    const ResourceManager& getResourceManager() const { return resourceManager; }
    bool userExists(const string& username) const { return users.find(username) != users.end(); }
};
...
protected:
    ...
    void displayMenuHeader() { cout << "\n┌────────────────────────────────────────────────────────┐\n"; }
    void displayMenuFooter() { cout << "└────────────────────────────────────────────────────────┘\n"; }
    void displayMenuOption(int number, const string& description) {
        cout << "│ " << number << ". " << description << string(51 - to_string(number).length() - description.length(), ' ') << "│\n";
    }
public:
    Portal(User* user, UserManager* manager) : currentUser(user), userManager(manager) {}
...
private:
    void displayUserList() {
        ...
        try {
            ...
            if (users.empty()) { printInfo("No users found"); return; }
            cout << "┌──────────────┬──────────────┬─────────┬──────────┬───────────┐\n";
            ...
            for (const auto& user : users) {
                ...
                printf("│ %-12s │ %-12s │ %7d │ %8d │ ✓ Full    │\n", user.getUsername().c_str(), user.getRole().c_str(), ram, storage);
            }
            cout << "└──────────────┴──────────────┴─────────┴──────────┴───────────┘\n";
    ...
    void createUserMenu() {
        ...
        cout << "└────────────────────────────────────────────────────────┘\n";
        try {
            if (password != passwordConfirm) { printError("Passwords do not match"); return; }
    ...
    void allocateResourcesMenu() {
        ...
        cout << "└────────────────────────────────────────────────────────┘\n";
        try {
            if (!userManager->userExists(username)) { printError("User not found"); return; }
    ...
    void manageFilesMenu() {
        ...
        try {
            if (command == "1") {
                ...
                if (files.empty()) { printInfo("No files found"); } else {
                    cout << "┌──────────────────────────────────────────────────────┐\n";
                    for (size_t i = 0; i < files.size(); ++i) printf("│ %-54s │\n", files[i].c_str());
                    cout << "└──────────────────────────────────────────────────────┘\n";
                }
            }
        } catch (const exception& e) { printError(e.what()); }
    ...
    void run() override {
        ...
        do {
            ...
            switch (choice) {
                ...
                case 3: deleteUserMenu(); break;
                case 4: allocateResourcesMenu(); break;
                case 5: manageFilesMenu(); break;
                case 6: printInfo("Logging out..."); break;
...
public:
    ...
    void run() override {
        ...
        do {
            ...
            cout << "└────────────────────────────────────────────────────────┘\n";
            switch (choice) {
                case 1: viewResourcesMenu(); break;
                case 2: manageFilesMenu(); break;
                case 3: printInfo("Logging out..."); break;
                default: printError("Invalid choice");
            }
        } while (choice != 3);
    }
...
public:
    ...
    void createFirstAdmin() {
        ...
        cout << "└────────────────────────────────────────────────────────┘\n";
        try {
            if (password != passwordConfirm) { printError("Passwords do not match"); createFirstAdmin(); return; }
            userManager.createUser(username, password, "Admin");
            printSuccess("Admin account '" + username + "' created successfully");
        } catch (const exception& e) { printError(e.what()); createFirstAdmin(); }
    }
    void login() {
        ...
        } catch (const exception& e) { printError(e.what()); login(); }
    }
    void selectPortal() {
        if (!currentUser) { printError("No user logged in"); return; }
        if (currentUser->isAdmin()) { AdminPortal adminPortal(currentUser, &userManager); adminPortal.run(); }
        else { EmployeePortal employeePortal(currentUser, &userManager); employeePortal.run(); }
        currentUser = nullptr;
    }
    void run() {
        try {
            ...
            while (running) {
                ...
                cin >> choice;
                cout << "└────────────────────────────────────────────────────────┘\n";
                if (choice != 'y' && choice != 'Y') running = false;
            }
            printHeader("Goodbye!");
            cout << "╔════════════════════════════════════════════════════════╗\n";
            cout << "║  " << string(19, ' ') << "Thank you for using our system!" << string(10, ' ') << "║\n";
            cout << "╚════════════════════════════════════════════════════════╝\n";
        } catch (const exception& e) { printError("Application error: " + string(e.what())); }
    }
};
int main() {
    try { Application app; app.run(); } catch (const exception& e) { cerr << "Fatal error: " << e.what() << endl; return 1; }
    return 0;
}
