  ...
  User(const string &u, const string &p, const string &r)
    ...
    if (!isValidPassword(p)) {
      throw invalid_argument(
          "Password must be at least 4 characters with at least one number");
    }
...
public:
  ...
  void allocateRam(const string &username, int amount) {
    if (amount < 0 || amount > MAX_RAM_PER_USER) {
      throw out_of_range("RAM must be between 0 and " +
                         to_string(MAX_RAM_PER_USER) + " MB");
    }
  ...
  void allocateStorage(const string &username, int amount) {
    if (amount < 0 || amount > MAX_STORAGE_PER_USER) {
      throw out_of_range("Storage must be between 0 and " +
                         to_string(MAX_STORAGE_PER_USER) + " MB");
    }
...
public:
  ...
  void loadUsers() {
    ...
    while (getline(file, line)) {
      ...
      if (!getline(ss, username, ',') || !getline(ss, encodedPassword, ',') ||
          !getline(ss, role, ',') || !getline(ss, ramStr, ',') ||
  ...
  void displayUserList() {
    ...
    try {
      ...
      for (const auto &user : users) {
        ...
        int storage =
            userManager->getResourceManager().getStorage(user.getUsername());

        printf("│ %-12s │ %-12s │ %7d │ %8d │ ✓ Full    │\n",
               user.getUsername().c_str(), user.getRole().c_str(), ram,
               storage);
      }
...
private:
  void viewResourcesMenu() {
    ...
    try {
      int ram =
          userManager->getResourceManager().getRam(currentUser->getUsername());
      int storage = userManager->getResourceManager().getStorage(
